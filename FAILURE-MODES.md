...
| F1 | Role escalation via client signup metadata | open | W1 — signup trigger must hardcode role, ignore `raw_user_meta_data` role |
...
| F3 | Stale JWT after promote/demote | open | W1 |
...
| F9 | RLS gap on a new table | open | W1 CI enumerator |
...
| F15 | Demoted IB still reads CRM via old session | open | W1 (`has_product()` in RLS) |
...
| F28 | `users.role` vs `platform_role` confusion | open | W1 (D8 doc comments) |
| F29 | Affiliate hits `/crm`, thinks it's broken | open | W1 (locked screen) |
